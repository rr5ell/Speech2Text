...
### Fixed
- 发布 `1.0.5+3`：新增右上角界面语言选择菜单，支持中文、英文、韩语、日语切换页面文案。
- 在首页左上角新增界面语言选择菜单，支持中文、English、한국어、日本語，并将状态卡和启动错误提示接入多语言文案。
- 发布 `1.0.4+2`：韩语距离测量补充 `저리`，并移除 pin catcher 中容易误触发的 `저`。
- 发布 `1.0.3+1`：补充中文测距热词 `车`，并将韩语 `캡챠` 调整为距离测量命中词。
